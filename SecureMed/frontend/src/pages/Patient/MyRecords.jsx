...
import { getSkPre } from "../../services/auth";
...
function SpinIcon()   { return <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="8" cy="8" r="6" strokeOpacity="0.25"/><path d="M8 2a6 6 0 016 6" strokeLinecap="round"><animateTransform attributeName="transform" type="rotate" from="0 8 8" to="360 8 8" dur="0.7s" repeatCount="indefinite"/></path></svg>; }
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ ...s, background: s.bg, fontSize: 12, padding: "3px 10px", borderRadius: 99, fontWeight: 600, whiteSpace: "nowrap" }}>
      {status}
    </span>
  );
}
...
function RecordCard({ record, onDelete }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const skOwner = getSkPre();
      if (!skOwner) {
        setErr("Encryption key not found in this browser. Did you register on a different device?");
        return;
      }

      const token = localStorage.getItem("token");
      const res   = await fetch(
        `http://localhost:5000/api/records/${record._id}/decrypt`,
        {
          method:  "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          // skOwner sent over TLS — never stored server-side
          body: JSON.stringify({ skOwner }),
        }
      );
      const data = await res.json();

      if (!res.ok || !data.success) {
        setErr(data.message || "Decryption failed");
        return;
      }

      downloadBase64(data.plaintext, data.fileName, data.mimeType);
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.borderColor = "#10b981"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(16,185,129,0.08)"; }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = "#e8eaed"; e.currentTarget.style.boxShadow = "none"; }}
    >
      {/* Top row */}
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        <div style={{ width: 34, height: 34, borderRadius: 9, flexShrink: 0, background: cfg.bg, color: cfg.color, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <TypeIcon type={record.type} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#111", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={record.title}>
            {record.title}
          </div>
          <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{record.type}</div>
        </div>
      ...
      {/* Inline error */}
      {err && (
        <div style={{ fontSize: 12, color: "#dc2626", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 7, padding: "6px 10px" }}>
          {err}
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button
            title={decrypting ? "Decrypting…" : "Download (decrypt)"}
            ...
            style={{
              ...
              color: decrypting ? "#10b981" : "#6b7280",
              transition: "all 0.15s",
            }}
            onMouseEnter={e => { if (!decrypting) { e.currentTarget.style.background = "#f0fdf4"; e.currentTarget.style.borderColor = "#10b981"; e.currentTarget.style.color = "#059669"; }}}
            onMouseLeave={e => { e.currentTarget.style.background = "none"; e.currentTarget.style.borderColor = "#e8eaed"; e.currentTarget.style.color = decrypting ? "#10b981" : "#6b7280"; }}
          >
            {decrypting ? <SpinIcon /> : <DownloadIcon />}
          </button>

          {/* Delete */}
          <button
            title="Delete"
            onClick={() => { if (window.confirm(`Delete "${record.title}"?`)) onDelete(record._id); }}
            style={{ width: 28, height: 28, borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "1px solid #e8eaed", cursor: "pointer", color: "#6b7280", transition: "all 0.15s" }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background = "none"; e.currentTarget.style.borderColor = "#e8eaed"; e.currentTarget.style.color = "#6b7280"; }}
          >
            <TrashIcon />
          </button>
        </div>
...
function StatCard({ label, value }) {
  return (
    <div style={{ flex: 1, minWidth: 100, background: "#fff", border: "1px solid #e8eaed", borderRadius: 12, padding: "14px 18px" }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.07em" }}>{label}</div>
      <div style={{ fontSize: 26, fontWeight: 700, color: "#111", marginTop: 4, letterSpacing: "-0.5px" }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative" }}>
            ...
            <input type="text" placeholder="Search records…" value={search} onChange={e => setSearch(e.target.value)}
              style={{ paddingLeft: 30, paddingRight: 10, width: 200, height: 34, fontSize: 13, border: "1px solid #e8eaed", borderRadius: 9, background: "#fff", color: "#374151", outline: "none" }} />
          </div>
          <select value={sort} onChange={e => setSort(e.target.value)}
            style={{ fontSize: 13, padding: "0 10px", height: 34, border: "1px solid #e8eaed", borderRadius: 9, background: "#fff", color: "#374151", cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {TYPE_FILTERS.map(type => (
          <button key={type} onClick={() => setFilter(type)} style={{
            fontSize: 13, padding: "5px 14px", borderRadius: 99, cursor: "pointer", border: "1px solid",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
        {filtered.length === 0 ? (
          <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "48px 0", fontSize: 14, color: "#9ca3af" }}>
            No records found.
          </div>
        ) : (
          filtered.map(record => (
